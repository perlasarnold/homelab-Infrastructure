...
* **Bulakan** (`VLAN 1 [Management]`)
* **Cebu** (`VLAN 1 [Management]`)
...
#### Source A: Bulakan Hypervisor (`VLAN 1 [Management]`)
...
#### Source B: Cebu Hypervisor (`VLAN 1 [Management]`)
...
### 1. Bastion VM (`VLAN 1 (Management)`) Traffic
```bash
tcpdump -n -i vmbr0 host VLAN 1 (Management) -c 50
```
* **Protocol**: **SMB (Port 445)**
* **Peer**: `VLAN 1 (Management)` (`pnas.local` — Synology NAS)
* **Secondary Traffic**: UDP traffic between the Bastion VM and `VLAN 1 (Management)` (`Perlas` client), characteristic of a remote session (RDP/Parsec).
* **Root Cause**: The user was active in the Bastion VM copying large volumes of data from the Synology NAS via SMB.

### 2. TrueNAS-SCALE VM (`VLAN 1 (Management)`) Traffic
```bash
tcpdump -n -i vmbr0 host VLAN 1 (Management) -c 50
```
* **Protocol**: **SMB (Port 445)**
* **Peer**: `VLAN 1 (Management)` (Local Windows Workstation)
* **Active Guest Commands**:
  ```bash
  ps aux | grep -E 'rclone|sync|rm'
  ```
  revealed:
  - `smbd: client [VLAN 1 (Management)]` consuming **10.3% CPU** (Sustained SMB access from the workstation).
...
1. **Active Transfers Identified**:
   - **~350 Mbps** is being consumed by **VM 203 (Bastion)** copying files from the **Synology NAS** (`VLAN 1 (Management)`).
   - **~70–115 Mbps** is being consumed by the **Windows Workstation** (`VLAN 1 (Management)`) interacting with **TrueNAS-SCALE** (`VLAN 1 (Management)`).
