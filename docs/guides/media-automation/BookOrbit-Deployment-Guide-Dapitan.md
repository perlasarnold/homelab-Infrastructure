...
**Date:** August 22, 2026  
**Objective:** Deploy BookOrbit on Proxmox node **Dapitan** (`VLAN 1 [Management]`), mount the Synology NAS ebook library `\\pnas\Media\Ebooks` persistently, configure SERVICES VLAN 110 segmentation, establish LAN DNS & SSL reverse proxy routing on `bookorbit.homelab-admin.me`, and configure the default administrator credentials.
...
|:---|:---|:---|
| **Hypervisor Node** | Dapitan | Dell OptiPlex 7050 SFF (`VLAN 1 [Management]`) |
| **Container Platform** | LXC CT 514 (`bookorbit-dapitan`) | Ubuntu 24.04 LTS, 2 vCPUs, 2048 MB RAM, 16 GB SSD (`vm-fast`) |
| **Network & VLAN** | **SERVICES VLAN 110** | IP: `VLAN 110 (Services)/24`, Gateway: `VLAN 110 (Services)`, DNS: `VLAN 1 [Secondary DNS]` |
| **Storage Source** | Synology PNAS SMB | `//VLAN 1 [Management]/Media/Ebooks` (`\\pnas\Media\Ebooks`) |
...
| **LAN DNS** | Pi-hole (`VLAN 1 [Secondary DNS]`) | `bookorbit.homelab-admin.me` $\rightarrow$ `VLAN 120 (DMZ)` |
...
3. Add the persistent mount entry to `/etc/fstab` on Dapitan:
   ```fstab
   //VLAN 1 [Management]/Media/Ebooks /mnt/bindmounts/ebooks cifs credentials=/root/.pnascredentials,iocharset=utf8,vers=3.0,uid=0,gid=0,file_mode=0775,dir_mode=0775,noperm,nobrl,_netdev,nofail,x-systemd.automount 0 0
   ```
...
1. Create unprivileged LXC container CT 514 on `vm-fast`:
   ...
   pct create 514 local:vztmpl/ubuntu-24.04-standard_24.04-2_amd64.tar.zst \
       ...
       --nameserver VLAN 1 [Secondary DNS] \
...
1. Open Pi-hole Admin Web Console (`http://VLAN 1 [Secondary DNS]/admin`).
