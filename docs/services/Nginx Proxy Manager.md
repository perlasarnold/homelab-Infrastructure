...
- **IP Address:** `VLAN 120 (DMZ)` (**DMZ VLAN 120**)
- **Admin Port:** `81` → `http://VLAN 120 (DMZ):81`
...
> - CT 502 (`VLAN 1 [Management]`) — was the primary; now offline permanently.
> - CT 102 (`VLAN 120 (DMZ)`) — legacy/secondary; now offline permanently.
> All proxy routing, SSL certificate management, and new proxy host configurations must be done on the **Cebu NPM** at `http://VLAN 120 (DMZ):81`.
...
| :--- | :--- | :--- | :--- | :--- | :--- |
| `auth.homelab-admin.me` | `http://VLAN 110 (Services):9000` | Authentik SSO | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
| `immich.homelab-admin.me`, `photos.homelab-admin.me` | `http://VLAN 110 (Services):2283` | Immich Photos (Dapitan CT 504) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets, 50GB Upload | 🟢 Online |
...
| `jellyfindp.homelab-admin.me` | `http://VLAN 110 (Services):8096` | Jellyfin Media Secondary (Dapitan CT 510) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Streaming Directives | 🟢 Online |
| `plex.homelab-admin.me`, `plexdp.homelab-admin.me` | `http://VLAN 110 (Services):32400` | Plex Media Server (Dapitan CT 509) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Streaming Directives | 🟢 Online (LAN Only) |
| `photoview.homelab-admin.me` | `http://VLAN 110 (Services):8000` | Photoview Photo Gallery (Dapitan CT 511) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | ⏳ Pending Deployment |
| `bookorbit.homelab-admin.me` | `http://VLAN 110 (Services):3000` | BookOrbit Ebook Platform (Dapitan CT 514) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
| `readarr.homelab-admin.me` | `http://VLAN 110 (Services):8787` | Readarr Book Automation (Cebu CT 417) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
| `sonarr.homelab-admin.me` | `http://VLAN 110 (Services):8989` | Sonarr TV Automation (Cebu CT 417) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
| `torrent.homelab-admin.me` | `http://VLAN 110 (Services):9091` | Transmission BitTorrent (Cebu CT 417) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
| `home.homelab-admin.me` | `http://VLAN 1 (Management):3000` | Homepage Dashboard (Bulakan CT 116) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
...
- **Task:** Configure Nginx Proxy Manager reverse proxy and Wildcard SSL routing for `home.homelab-admin.me` (`VLAN 1 (Management):3000`) and `torrent.homelab-admin.me` (`VLAN 110 (Services):9091`).
- **Actions Taken:**
  - Deployed `/data/nginx/proxy_host/16.conf` (`torrent.homelab-admin.me`) and `/data/nginx/proxy_host/17.conf` (`home.homelab-admin.me`) on Cebu CT 105 (`VLAN 120 (DMZ)`).
...
- **Task:** Configure Nginx Proxy Manager reverse proxy routing for `sonarr.homelab-admin.me` pointing to Cebu Arr Stack CT 417 (`http://VLAN 110 (Services):8989`).
...
- **Actions Taken:**
  - Deployed `/data/nginx/proxy_host/15.conf` on Cebu CT 105 (`VLAN 120 (DMZ)`).
...
- **Task:** Configure Nginx Proxy Manager reverse proxy routing for `bookorbit.homelab-admin.me` pointing to Dapitan BookOrbit CT 514 (`http://VLAN 110 (Services):3000`).
...
- **Actions Taken:**
  - Deployed `/data/nginx/proxy_host/13.conf` on Cebu CT 105 (`VLAN 120 (DMZ)`).
...
- **Root Cause:** NPM proxy host definitions only included `photos.homelab-admin.me` and `jellyfindp.homelab-admin.me`. When Firefox queried local Pi-hole DNS (`VLAN 120 (DMZ)`), SNI mismatches caused Nginx to fall back to the default server block without valid certificates, aborting the TLS handshake.
- **Actions Taken:**
  - Added `immich.homelab-admin.me` alias to Proxy Host 9.
  - Added `jellyfin.homelab-admin.me` and `media.homelab-admin.me` aliases to Proxy Host 11 (`VLAN 110 (Services):8096`).
  - Added `plex.homelab-admin.me` alias to Proxy Host 12 (`VLAN 110 (Services):32400`).
  - Decommissioned obsolete Host 10 pointing to offline node `VLAN 1 [Management]`.
...
- **Task:** Migrate Dapitan Plex (CT 509) to SERVICES VLAN 110 (`VLAN 110 (Services):32400`) and establish reverse proxy routing on `plexdp.homelab-admin.me`.
...
- **Actions Taken:**
  - Migrated CT 509 from VLAN 1 (`VLAN 1 (Management)`) to SERVICES VLAN 110 (`VLAN 110 (Services)/24`) with `hwaddr=00:11:22:33:44:55` preserved.
  - Created DNS-Only `A` record on Cloudflare for `plexdp.homelab-admin.me` pointing to public WAN IP.
  - Added Pi-hole local DNS override mapping `plexdp.homelab-admin.me` $\rightarrow$ `VLAN 120 (DMZ)` (NPM Cebu).
  ...
  - Updated Plex `Preferences.xml` with `customConnections="https://plexdp.homelab-admin.me:443"`, `lanNetworks="VLAN 110 (Services)/24,VLAN 1 (Management)/24,VLAN 120 (DMZ)/24"`, and `RelayEnabled="0"`.
...
- **Proxy Hosts Configured:**
  - `auth.homelab-admin.me` $\rightarrow$ `http://VLAN 110 (Services):9000` (Authentik)
  - `photos.homelab-admin.me` $\rightarrow$ `http://VLAN 110 (Services):2283` (Immich)
  - `media.homelab-admin.me` $\rightarrow$ `http://VLAN 1 [Management]:8096` (Jellyfin)
...
- **Cloudflare Configuration:**
    ...
    - Pointed the service to `http://VLAN 1 [Management]:80`.
...
- [ ] Migrate Plex (`VLAN 1 [Management]:32400`) and Audiobookshelf (`VLAN 1 [Management]:13378`) to NPM routing.
