...
# Copy the contents of \\pnas\Seagate\Share\Movies to Dapitan's
# bulk18/media-data library. The SMB source is always mounted read-only.
#
# Safety defaults:
...
readonly SOURCE_SUBDIR="${SOURCE_SUBDIR:-Share/Movies}"
readonly SOURCE_MOUNT="${SOURCE_MOUNT:-/mnt/source-pnas-seagate}"
readonly DESTINATION="${DESTINATION:-/mnt/bindmounts/media-data/library/movies}"
...
readonly LOG_DIR="${LOG_DIR:-/var/log/dapitan-pnas-movies-copy}"
readonly LOCK_FILE="${LOCK_FILE:-/run/lock/dapitan-pnas-movies-copy.lock}"
...
usage() {
  cat <<'EOF'
Usage:
  dapitan-copy-pnas-movies.sh --check
  dapitan-copy-pnas-movies.sh [--dry-run] [--bwlimit-kib N]
  dapitan-copy-pnas-movies.sh --execute [--bwlimit-kib N]
...
exec 9>"$LOCK_FILE"
flock -n 9 || fail "another Dapitan movie-copy process is already active."
...
if ionice -c 2 -n 7 nice -n 10 \
  ...
  if [[ "$mode" == "execute" ]]; then
    ...
    pct exec 509 -- su - plex -s /bin/bash -c "export LD_LIBRARY_PATH=/usr/lib/plexmediaserver/lib; /usr/lib/plexmediaserver/Plex\ Media\ Scanner --scan --section 1" >/dev/null 2>&1 || true
