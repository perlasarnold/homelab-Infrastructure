...
2. Add the following records pointing to the NPM IP (e.g., `VLAN 1 [Management]`):
   - `sonarr.local` -> `VLAN 1 [Management]`
   - `radarr.local` -> `VLAN 1 [Management]`
   - `bazarr.local` -> `VLAN 1 [Management]`
   - `wizarr.local` -> `VLAN 1 [Management]`
   - `prowlarr.local` -> `VLAN 1 [Management]`
   - `jackett.local` -> `VLAN 1 [Management]`
...
2. Create an entry for each service, pointing to the Arr LXC IP (`VLAN 1 (Management)`):
   - `sonarr.local` -> `VLAN 1 (Management):8989`
   - `radarr.local` -> `VLAN 1 (Management):7878`
   - `bazarr.local` -> `VLAN 1 (Management):6767`
   - `wizarr.local` -> `VLAN 1 (Management):5690`
   - `prowlarr.local` -> `VLAN 1 (Management):9696`
   - `jackett.local` -> `VLAN 1 (Management):9117`
