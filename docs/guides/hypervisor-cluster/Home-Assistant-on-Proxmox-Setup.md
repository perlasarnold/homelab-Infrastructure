...
* **Date:** May 18, 2026  
* **Objective:** Deploy Home Assistant OS (HAOS) as a high-performance, resilient Virtual Machine on the `cebu` Proxmox VE node (`VLAN 1 [Management]`) utilizing the community-maintained Proxmox VE Helper Script (`haos-vm.sh`) for rapid, standardized deployment.
* **Maintainer:** Perlas  
...
* **Host Node:** `cebu` (VLAN 1 [Management])
* **VM ID:** `111`
* **VM Name:** `haos-17.3`
* **Active IP Address:** `VLAN 1 [Management]` (Dynamically reported by QEMU Guest Agent)
...
2. Log in to the Proxmox Web GUI (`https://VLAN 1 [Management]:8006`).
...
2. Once booted, the terminal displays the local network IP and port:
   ```
   IPv4 addresses for enp6s18: VLAN 1 [Management]/24
   Home Assistant URL:        http://homeassistant.local:8123
   Observer URL:              http://homeassistant.local:4357
   ```
3. Open a web browser on any computer connected to your local network and navigate to `http://VLAN 1 [Management]:8123` or `http://homeassistant.local:8123`.
...
* **Recommended (DHCP Reservation):** Log in to your router/firewall dashboard, find the DHCP lease list, match the MAC address of VM 111 (visible in Proxmox > VM 111 > Hardware > Network Device), and reserve `VLAN 1 [Management]` as the static assignment.
* **Inside Home Assistant (Fallback):** 
  ...
  4. Configure:
     ...
     * **DNS Servers:** `VLAN 1 [Primary DNS]` (Pi-hole on Cebu node) and `1.1.1.1` (Backup)
...
3. [[02-Proxmox/Proxmox Overview]] — Hardware profile of node Cebu (`VLAN 1 [Management]`).
