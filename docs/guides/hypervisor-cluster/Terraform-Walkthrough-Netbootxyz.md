...
1. **Open Proxmox Web UI:** https://VLAN 1 [Management]:8006
...
```bash
ssh root@VLAN 1 [Management] ls -la /var/lib/vz/template/cache/
```
...
4. Proxmox:
   ...
   - Configures network (VLAN 1 [Management])
...
1. **Open browser:** http://VLAN 1 [Management]:3000
...
│  (blank/new) │   ┌──────────────►  │  (netboot)   │
│              │   │  "I need OS!"   │  VLAN 1 [Management]│
└──────────────┘   │                  └──────────────┘
      ▲            │
      │            │ Step 2: "DHCP says TFTP is at VLAN 1 [Management]"
      │            │
...
1. **Login to Pi-hole:** http://VLAN 1 (Management)/admin (or check CT 301's IP)
2. **Settings** → **DHCP** tab
...
**Why this works:** When a PXE client boots, it asks DHCP: "Who's the TFTP server?" Pi-hole replies: "VLAN 1 [Management]" and the client downloads the boot files.

#### Option B: Router DHCP

Every router is different, but look for:
- **Boot Server** or **TFTP Server:** `VLAN 1 [Management]`
...
The Web UI (http://VLAN 1 [Management]:3000) lets you:
