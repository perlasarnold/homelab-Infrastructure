...
> **Local DNS:** Pi-hole Primary (`VLAN 1 [DNS-Primary]` CT 301) & Secondary (`VLAN 1 (Mgmt)` CT 401)  
...
Executed on host `Dapitan` (`VLAN 1 [MGMT]`):
```bash
...
- `lanNetworks="VLAN 110 (Services)/24,192.168.1.0/24,VLAN 120 (DMZ)/24"`
- `RelayEnabled="0"`
...
1. **Revert CT 509 Network:**
   ```bash
   pct set 509 -net0 name=eth0,bridge=vmbr0,gw=VLAN 1 [Gateway],hwaddr=00:11:22:33:44:55,ip=VLAN 1 (Mgmt)/24,type=veth
   pct reboot 509
   ```
