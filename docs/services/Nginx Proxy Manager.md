...
> - CT 502 (`VLAN 1 (Mgmt)`) — was the primary; now offline permanently.
...
| `home.homelab-admin.me` | `http://VLAN 1 (Mgmt):3000` | Homepage Dashboard (Bulakan CT 116) | `*.homelab-admin.me` | Force SSL, HTTP/2, HSTS, Websockets | 🟢 Online |
...
- **Task:** Configure Nginx Proxy Manager reverse proxy and Wildcard SSL routing for `home.homelab-admin.me` (`VLAN 1 (Mgmt):3000`) and `torrent.homelab-admin.me` (`VLAN 110 (Services):9091`).
- **Actions Taken:**
...
- **Actions Taken:**
  ...
  - Decommissioned obsolete Host 10 pointing to offline node `VLAN 1 (Mgmt)`.
...
- **Actions Taken:**
  - Migrated CT 509 from VLAN 1 (`VLAN 1 (Mgmt)`) to SERVICES VLAN 110 (`VLAN 110 (Services)/24`) with `hwaddr=00:11:22:33:44:55` preserved.
  ...
  - Updated Plex `Preferences.xml` with `customConnections="https://plexdp.homelab-admin.me:443"`, `lanNetworks="VLAN 110 (Services)/24,192.168.1.0/24,VLAN 120 (DMZ)/24"`, and `RelayEnabled="0"`.
...
- **Proxy Hosts Configured:**
  ...
  - `media.homelab-admin.me` $\rightarrow$ `http://VLAN 1 (Mgmt):8096` (Jellyfin)
...
- **Cloudflare Configuration:**
    ...
    - Pointed the service to `http://VLAN 1 (Mgmt):80`.
...
- [ ] Migrate Plex (`VLAN 1 (Mgmt):32400`) and Audiobookshelf (`VLAN 1 (Mgmt):13378`) to NPM routing.
