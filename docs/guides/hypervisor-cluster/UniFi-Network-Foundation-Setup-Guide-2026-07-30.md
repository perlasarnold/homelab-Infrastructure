...
|:---:|:---|:---|:---|:---|:---|
| **10** | **MGMT** | `VLAN 10 (SecOps)/24` | `VLAN 10 (SecOps)` | `VLAN 10 (SecOps) - 249` | Proxmox hypervisors, UniFi gateway, switches, APs, Synology DSM & TrueNAS web GUIs |
...
| **30** | **IOT** | `VLAN 30 (IoT)/24` | `VLAN 30 [Gateway]` | `VLAN 30 (IoT) - 249` | Home Assistant OS (`haos-17.3`), smart TVs, IoT sensors, cameras |
...
* **Rationale:** Establishes the gateway IP addresses on the UCG Max router without affecting active hosts on native VLAN 1 (`VLAN 1 (Management)/24`).
...
* **Action:** Created stateful Firewall Groups in UniFi (`Settings` ➡️ `Security` ➡️ `Firewall Rules` ➡️ `IP/Port Groups`):
  ...
  * **IP Group `Storage Hosts`:** Synology (`VLAN 1 [Management]`, `.13`) & TrueNAS (`VLAN 1 (Management)`)
  * **IP Group `DNS Resolvers`:** Pi-hole Primary (`VLAN 1 [Secondary DNS]` / `VLAN 110 (Services)`) & Secondary (`VLAN 1 (Management)` / `VLAN 110 (Services)`)
