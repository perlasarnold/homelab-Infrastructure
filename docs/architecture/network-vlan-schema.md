...
| `192.168.1.246` | tailscale-cebu (CT 407) | Tailscale Subnet Router & Exit Node (Cebu) | VLAN 1 (Mgmt) | 🟢 Active |
| `192.168.10.50` | fail2ban-cebu (CT 406) | Fail2Ban IPS & Dashboard (Cebu) | VLAN 10 (SecOps) | 🟢 Active |
...
| `192.168.110.60` | grafana-cebu (CT 418) | Grafana Observability Dashboard (Cebu) | VLAN 110 (Services) | 🟢 Active |
| `192.168.110.61` | uptime-kuma-cebu (CT 419) | Uptime Kuma Service Monitor (Cebu) | VLAN 110 (Services) | 🟢 Active |
...
| Upstream | `1.1.1.1` / `8.8.8.8` | Cloudflare / Google | Upstream Anycast Resolvers |

### Split-Horizon DNS Rules
- **Internal Homelab Subdomains:** Mapped explicitly in `/etc/pihole/custom.list` (`auth.perlasarnold.me`, `jellyfin.perlasarnold.me`, `plexdp.perlasarnold.me`, `immich.perlasarnold.me`, etc.) directly to Nginx Proxy Manager (`192.168.120.211`) for 0ms local LAN loopback.
- **Apex Domain & GitHub Pages Portfolio:** Configured in `/etc/dnsmasq.d/99-perlasarnold-lan.conf` to forward queries to upstream public DNS (`server=/perlasarnold.me/#`, `server=/www.perlasarnold.me/#`, `server=/perlasarnold.github.io/#`). This allows public GitHub Pages hosting and Cloudflare redirect rules to function without local SSL certificate collisions.
...
- `enx6c1ff706209e` — USB Realtek RTL8152 NIC (backup)
...
| 110 | SERVICES | `192.168.110.0/24` | `192.168.110.1` | Internal Homelab Services (Plex, Arr stack, Immich) |
...
- [Class C Subnet & IP Allocation Schema Recommendation](class-c-subnet-schema.md)
- [UniFi Preemptive Foundation Setup Guide](unifi-preemptive-foundation.md)
- [UniFi Router & Network Setup](unifi-gateway-setup.md)
- [Proxmox Cluster Topology](cluster-topology.md)
- [Master Services Index](../services/services-index.md)
- [Split-Horizon DNS Troubleshooting Guide](../guides/hypervisor-cluster/PerlasArnold-Me-Split-Horizon-DNS-Resolution-Guide-2026-08-22.md)
