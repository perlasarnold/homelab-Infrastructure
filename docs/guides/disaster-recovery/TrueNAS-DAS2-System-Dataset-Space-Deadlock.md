...
The TrueNAS SCALE (VM 120) SMB service (`smbd`) stopped responding to connection requests. Windows client connection attempts to the share `\\VLAN 1 (Management)\photo` failed with network path/access errors.
...
3. **SMB Connectivity Restored**: Validated that clients can connect and browse the share from Windows Powershell:
   ```powershell
   Get-ChildItem \\VLAN 1 (Management)\photo
   ```
