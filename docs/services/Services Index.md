...
## Proxmox LXC Services (Bulakan — VLAN 1 [Management])

| Service | CT ID | VLAN / Subnet | IP Address | Status | Purpose |
|:---|:---:|:---:|:---:|:---:|:---|
| Audiobookshelf | 100 | VLAN 1 (Mgmt) | `VLAN 1 [Management]` | 🟢 Running | Audiobook server |
| Plex (Bulakan) | 104 | VLAN 1 (Mgmt) | `VLAN 1 [Management]` | 🟢 Running | Media server (Bulakan, 9th Gen QSV GPU) |
| Jackett | 108 | VLAN 1 (Mgmt) | `VLAN 1 [Management]` | 🟢 Running | Secondary tracker proxy |
...
| Pi-hole (Bulakan) | 301 | VLAN 1 (Mgmt) | `VLAN 1 [Primary DNS]` | 🟢 Running | Primary DNS ad-blocker & local resolver |
...
| Netboot.xyz | 118 | VLAN 1 (Mgmt) | `VLAN 1 [Management]` | ⚪ Stopped | PXE network boot installer |

---

## Proxmox LXC Services (Cebu — VLAN 1 [Management])
...
| Pi-hole (Cebu) | 401 | VLAN 1 (Mgmt) | `VLAN 1 [Secondary DNS]` | 🟢 Running | Secondary DNS Failover Resolver |
...
## Proxmox LXC Services (Dapitan — VLAN 1 [Management])
...
| Home Assistant OS (HAOS) | VM 111 | VLAN 1 (Mgmt) | `VLAN 1 [Management]` | 🟢 Running | Smart Home Controller |

---

## Proxmox VMs (Dapitan — VLAN 1 [Management])
...
## 🛑 DEPRECATED (Unraid — VLAN 1 (Management))
...
### 📺 Media
- Jellyfin → http://VLAN 1 [Management]:8096 (Bulakan) / Cebu node (Replacing Unraid)
- Plex → Bulakan CT 104 (Proxmox LXC)
- Calibre-Web → http://VLAN 1 (Management):8083 (Bulakan CT 113)
...
- Prowlarr → http://VLAN 1 (Management):9696 (Bulakan)
...
- Pi-hole → Bulakan CT 301 + Cebu CT 401 (VLAN 1 [Primary DNS])
- Cloudflared → Bulakan CT 304
- PNAS (Synology) → http://VLAN 1 [Management]:5001 (Shared PVE Storage)
- Netboot.xyz → Bulakan CT 118 → http://VLAN 1 [Management]:3000
- Authentik → https://auth.homelab-admin.me (SSO / IAM)
- Nginx Proxy Manager → http://VLAN 120 (DMZ):81 (Admin Dashboard — Cebu CT 105, **Active**) ~~http://VLAN 1 [Management]~~ (Bulakan CT 502, **OFFLINE**)~~

### 🏠 Dashboards
- Homepage → https://home.homelab-admin.me (Bulakan CT 116 — `VLAN 1 (Management):3000`)
