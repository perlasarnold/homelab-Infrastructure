#!/usr/bin/env bash
# ==============================================================================
# Plex Automated Update with 5-Point QA Health Validation & Auto-Rollback
...
#   2. Runs APT package upgrade for plexmediaserver inside LXC container.
#   3. Executes 5-Point QA Health Suite:
#        - Check 1: systemd service active (plexmediaserver)
#        - Check 2: HTTP 200 OK from /web/index.html
#        - Check 3: Plex API /identity response with machineIdentifier
#        - Check 4: Intel GPU hardware acceleration device accessible (vainfo)
#        - Check 5: /dev/shm/transcodes RAM disk writable by plex user
#   4. IF ALL QA CHECKS PASS:
...
LOG_FILE="/var/log/plex-auto-update.log"
...
log "========================================================================"
log "Starting Automated Plex Upgrade & QA Suite for Container: ${CT_ID}"
...
PRE_VERSION=$(pct exec "${CT_ID}" -- dpkg -s plexmediaserver 2>/dev/null | grep '^Version:' | awk '{print $2}' || echo "unknown")
log "Current installed Plex version: ${PRE_VERSION}"
...
log "Checking and applying Plex updates via APT..."
pct exec "${CT_ID}" -- apt-get update -qq

# Check if an update is actually available
UPGRADEABLE=$(pct exec "${CT_ID}" -- apt-get --just-print upgrade | grep -i plexmediaserver || true)
if [ -z "${UPGRADEABLE}" ]; then
    log "Plex is already at the latest version (${PRE_VERSION}). No upgrade needed."
...
if ! pct exec "${CT_ID}" -- apt-get install --only-upgrade -y -qq plexmediaserver; then
...
POST_VERSION=$(pct exec "${CT_ID}" -- dpkg -s plexmediaserver | grep '^Version:' | awk '{print $2}')
log "Plex package updated: ${PRE_VERSION} -> ${POST_VERSION}"
...
for i in $(seq 1 12); do
    sleep 5
    log "QA Attempt ${i}/12: Validating service health..."
    ...
    # Check 2: HTTP 200 OK on Web UI
    HTTP_CODE=$(pct exec "${CT_ID}" -- curl -s -o /dev/null -w "%{http_code}" http://127.0.0.1:32400/web/index.html || echo "000")
    if [ "${HTTP_CODE}" != "200" ]; then
        log "  [QA-2] Web UI returned HTTP ${HTTP_CODE} (expected 200), waiting..."
        continue
    fi

    # Check 3: Plex API Identity Response
    if ! pct exec "${CT_ID}" -- curl -s http://127.0.0.1:32400/identity | grep -q "MediaContainer"; then
        log "  [QA-3] Plex API /identity not responding with valid XML, waiting..."
        continue
    fi

    # Check 4: GPU Device Passthrough & VA-API
    if pct exec "${CT_ID}" -- test -e /dev/dri/renderD128; then
        if ! pct exec "${CT_ID}" -- su - plex -s /bin/bash -c "vainfo --display drm --device /dev/dri/renderD128" >/dev/null 2>&1; then
            log "  [QA-4] vainfo GPU check failed under plex user, retrying..."
            continue
        fi
    fi

    # Check 5: /dev/shm/transcodes Writable
    pct exec "${CT_ID}" -- mkdir -p /dev/shm/transcodes
    pct exec "${CT_ID}" -- chown -R plex:plex /dev/shm/transcodes
    if ! pct exec "${CT_ID}" -- su - plex -s /bin/bash -c "touch /dev/shm/transcodes/.qa_test && rm /dev/shm/transcodes/.qa_test"; then
        log "  [QA-5] RAM disk /dev/shm/transcodes write test failed, retrying..."
        continue
    fi
...
if [ "${QA_PASSED}" = true ]; then
    ...
    log " Successfully upgraded Plex: ${PRE_VERSION} -> ${POST_VERSION}"
    log "========================================================================"
    ...
    error_log "========================================================================"
    error_log " [QA FAILED] Plex health checks failed after upgrade on CT ${CT_ID}."
