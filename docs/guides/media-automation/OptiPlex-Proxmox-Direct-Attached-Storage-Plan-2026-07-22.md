...
- **Status:** Implementation in progress on `Dapitan` (`VLAN 1 [Management]`). Base storage was created and verified on 2026-07-22, and Dapitan joined the three-node `Homelab-Net` cluster on 2026-07-23. Extended disk tests, BIOS update, permissions, workload deployment, and data migration remain pending.
...
| Network | Intel I219-LM | `e1000e`; bridge `vmbr0`; static IP `VLAN 1 [Management]/24` |
...
| Check | Result |
|---|---|
| Existing cluster before join | `Homelab-Net`, nodes `Bulakan` (`VLAN 1 [Management]`) and `cebu` (`VLAN 1 [Management]`) |
...
5. Joined Dapitan to `VLAN 1 [Management]`.
...
- Dapitan can reach `VLAN 1 [Management]` on TCP port 445.
