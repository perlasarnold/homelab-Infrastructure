...
> **Target Host:** Proxmox Node `Dapitan` (`VLAN 1 (Management)`)  
> **Maintainer:** Perlas  
...
- **VM Specs**: `torrent-box-dapitan` | VM ID `501` | Static IP **`VLAN 1 [Management]/24`** | Host `dapitan` (`VLAN 1 (Management)`)
...
2. **QEMU Guest Agent**: Verify IP `VLAN 1 [Management]` is reported under VM Summary.
