...
| :--- | :--- | :--- |
| **Host Node** | `Dapitan` (VLAN 1 [Management]) | Node 3 in `Homelab-Net` cluster; offloads storage/compute from Bulakan. |
...
| **IP Address** | `VLAN 1 (Management)` | DHCP assigned static lease / container address. |
...
1. **Source Mounting**: Mounted Bulakan VM 204 ZFS volume partition read-only at `/mnt/immich-vm-temp` on host Bulakan (`VLAN 1 [Management]`), keeping VM 204 online without write locks.
2. **Library Asset Transfer**: Executed `rsync` from Bulakan host directly to Dapitan's 18TB dataset `/mnt/bindmounts/immich-data/`:
   ...
   nohup rsync -av --info=progress2 \
     ...
     root@VLAN 1 [Management]:/mnt/bindmounts/immich-data/ \
...
### Service Health
- **HTTP Endpoint**: `http://VLAN 1 (Management):2283` returns `HTTP 200 OK`.
...
- Dapitan reached `http://VLAN 1 (Management):2283` with HTTP 200.
...
2. **Reverse Proxy Update**: Update Nginx Proxy Manager (`VLAN 1 [Management]`) upstream target for Immich from `VLAN 1 (Management)` to `VLAN 1 (Management)`.
