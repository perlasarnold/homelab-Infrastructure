...
Provision and replicate the **Jellyfin Media Server** container (`jellyfin-dapitan`, LXC Container `510`) on the **Dapitan** Proxmox VE host (`VLAN 1 [MGMT]`) from the existing Cebu Jellyfin server (CT `416`), attaching the local 18TB ZFS media dataset path `/mnt/bindmounts/media-data/library` (`\\VLAN 1 [MGMT]\media-data\library`).
...
|---|---|---|
| **Host Node** | `Dapitan` (`VLAN 1 [MGMT]`) | Node 3 in `Homelab-Net` cluster |
...
| **IP Address** | `VLAN 1 (Mgmt)` | Configured on `vmbr0` bridge |
| **Service Listening Port** | `8096` (TCP) | Default Jellyfin HTTP port |
| **Web Access URL** | **[http://VLAN 1 (Mgmt):8096](http://VLAN 1 (Mgmt):8096)** | Web interface URL |
...
- **Issue**: Initial static IP `VLAN 1 (Mgmt)` caused TCP connection resets (`ERR_CONNECTION_REFUSED`) from local network clients due to router ARP table routing behavior for static IPs outside the router's active DHCP range.
- **Resolution**: Updated container network interface `net0` to IP `VLAN 1 (Mgmt)/24` (Gateway `VLAN 1 [Gateway]`), created `/usr/share/jellyfin/web` to `/var/lib/jellyfin/wwwroot` static asset symlink, and set explicit local network subnet permissions in `/etc/jellyfin/network.xml`.
- **Result**: External LAN requests return `HTTP 200 OK` instantly on `http://VLAN 1 (Mgmt):8096`.
...
pct create 510 PNAS:vztmpl/debian-12-standard_12.7-1_amd64.tar.zst \
  ...
  --net0 name=eth0,bridge=vmbr0,ip=VLAN 1 (Mgmt)/24,gw=VLAN 1 [Gateway] \
...
ssh root@VLAN 1 [MGMT] "pct exec 510 -- systemctl stop jellyfin"
ssh root@VLAN 1 [MGMT] "pct exec 416 -- systemctl stop jellyfin"

# Transfer data structures
ssh root@VLAN 1 [MGMT] "pct exec 416 -- tar -cf - /var/lib/jellyfin /etc/jellyfin" | \
  ssh root@VLAN 1 [MGMT] "pct exec 510 -- tar -xf - -C /"

# Restart Cebu Jellyfin
ssh root@VLAN 1 [MGMT] "pct exec 416 -- systemctl start jellyfin"
```
...
- **LAN Web Interface Test**: `(Invoke-WebRequest -Uri 'http://VLAN 1 (Mgmt):8096/web/index.html').StatusCode` returned `200`.
...
Dapitan Jellyfin Media Server (CT `510`) is online and accessible at **[http://VLAN 1 (Mgmt):8096](http://VLAN 1 (Mgmt):8096)** with all user accounts and configuration cloned from Cebu, attached directly to host dataset `\\VLAN 1 [MGMT]\media-data\library` (`/media/library`).
