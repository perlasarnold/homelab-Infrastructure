...
| Layer | Path |
|---|---|
| Synology SMB source | `//VLAN 1 [Management]/Media/Ebooks` |
...
4. Added the persistent host mount to `/etc/fstab`:
   ...
   ```fstab
   //VLAN 1 [Management]/Media/Ebooks /mnt/ebooks cifs credentials=/root/.pnascredentials,iocharset=utf8,vers=3.0,uid=100000,gid=100000,file_mode=0664,dir_mode=0775,noperm,nobrl,_netdev,nofail,x-systemd.automount 0 0
   ```
...
- Host mount active: `//VLAN 1 [Management]/Media/Ebooks` at `/mnt/ebooks`.
