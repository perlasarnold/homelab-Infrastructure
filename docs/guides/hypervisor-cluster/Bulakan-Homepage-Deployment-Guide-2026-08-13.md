...
- **Date:** August 13, 2026
- **Objective:** Provision a dedicated Homepage (`gethomepage.dev`) dashboard running in a Debian 12 LXC container (**CT 116**) on the Bulakan Proxmox VE node (`VLAN 10 (MGMT/SecOps)`), accessible externally via Cloudflare Tunnel at `https://home.homelab-admin.me`.
- **Maintainer:** Perlas
...
| :--- | :--- | :--- |
| **Host Node** | `Bulakan` (`VLAN 10 (MGMT/SecOps)`) | Primary hypervisor node in the cluster. |
...
Execute on **Bulakan PVE** (`VLAN 10 (MGMT/SecOps):8006`) via PVE Web Shell or SSH:
