...
During a proactive security audit, several Python scripts inside the `scripts/` directory were identified as containing plaintext credentials for the root account on Proxmox nodes (`VLAN 1 [Management]` and `VLAN 1 [Management]`):
