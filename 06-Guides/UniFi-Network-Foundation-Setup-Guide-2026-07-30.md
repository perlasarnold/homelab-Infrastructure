...
|:---:|:---|:---|:---|:---|:---|
| **10** | **MGMT** | `192.168.10.0/24` | `192.168.10.1` | `192.168.10.200 - 249` | Proxmox hypervisors, UniFi gateway, switches, APs, Synology DSM & TrueNAS web GUIs |
| **20** | **TRUSTED** | `192.168.20.0/24` | `192.168.20.1` | `192.168.20.100 - 249` | Admin workstations (`Perlas-W10`), daily laptops, mobile devices |
| **30** | **IOT** | `192.168.30.0/24` | `192.168.30.1` | `192.168.30.100 - 249` | Home Assistant OS (`haos-17.3`), smart TVs, IoT sensors, cameras |
| **110** | **SERVICES** | `192.168.110.0/24` | `192.168.110.1` | `192.168.110.100 - 249` | Internal homelab services (Plex, Jellyfin, Arr stack, Immich, Fileserver, Authentik) |
| **120** | **DMZ** | `192.168.120.0/24` | `192.168.120.1` | `192.168.120.100 - 249` | Public ingress controllers (Cloudflared tunnels, Nginx Proxy Manager) |
...
* **Action:** Created stateful Firewall Groups in UniFi (`Settings` ➡️ `Security` ➡️ `Firewall Rules` ➡️ `IP/Port Groups`):
  ...
  * **IP Group `Storage Hosts`:** Synology (`192.168.1.12`, `.13`) & TrueNAS (`192.168.1.211`)
  * **IP Group `DNS Resolvers`:** Pi-hole Primary (`192.168.1.5` / `192.168.110.5`) & Secondary (`192.168.1.6` / `192.168.110.6`)
...
4. **DMZ Ingress Isolation:** Primary `cloudflared` (Bulakan CT 304 — `192.168.120.7`), secondary `cloudflared-cebu` (Cebu CT 404 — `192.168.120.6`), and Nginx Proxy Manager instances are active in **DMZ (VLAN 120)**.
5. **Services Zone Integration:** Primary `immich-dapitan` (Dapitan CT 504 — `192.168.110.47`) is active in **SERVICES (VLAN 110)** with verified external Cloudflare Tunnel routing.
...
1. Open UniFi Network Controller (`https://192.168.1.1`).
