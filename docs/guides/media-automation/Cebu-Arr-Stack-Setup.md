...
| :--- | :--- | :--- |
| **Host Node** | `Cebu` (VLAN 1 [MGMT]) | Workload distribution across the homelab cluster. |
...
3. Append the CIFS mount to `/etc/fstab` (using your existing `/etc/samba/credentials-seagate` file):
   ```text
   //VLAN 1 (Mgmt)/seagate /mnt/truenas/seagate cifs credentials=/etc/samba/credentials-seagate,iocharset=utf8,uid=100000,gid=100000,file_mode=0775,dir_mode=0775,nofail 0 0
   ```
...
## ✅ Outcome
- The Debian LXC container (`arr-stack-cebu`, IP `VLAN 1 (Mgmt)`) was successfully provisioned with Nesting and Keyctl enabled.
