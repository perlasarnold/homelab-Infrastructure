...
- **Target Host:** Proxmox VE Node 3 `Dapitan` (`VLAN 1 [Management]` / `VLAN 10 (SecOps)`)
...
- **Recommended Static IP:** `VLAN 20 (Trusted)` *(or `VLAN 1 (Management)` during flat-network transition)*
...
#### Why VLAN 20 (TRUSTED)?
1. **Administrative Jump Host Capability:** Once connected to the Linux Mint desktop via Guacamole, you need the capability to manage Proxmox VE nodes (`192.168.10.x`), Synology/TrueNAS interfaces (`VLAN 10 (SecOps)`), and service containers (`192.168.110.x`). VLAN 20 is designated for trusted admin endpoints.
...
| :--- | :--- | :--- |
| **Target Proxmox Node** | `Dapitan` (`VLAN 1 [Management]` / `VLAN 10 (SecOps)`) | Node 3 in Homelab-Net cluster |
...
4. **In Guacamole Web UI (`http://VLAN 1 (Management):8080/guacamole/`):**
