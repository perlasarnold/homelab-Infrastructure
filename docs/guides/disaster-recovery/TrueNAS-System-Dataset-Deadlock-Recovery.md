...
During the final cutover of the photo storage migration from `DAS1-18TB` to `DAS2-18TB`, the Samba service (`smbd`) on TrueNAS SCALE (VM 120) crashed and refused to start. Attempts to mount the SMB share `//VLAN 1 (Mgmt)/photo` on the Proxmox Cebu host (`VLAN 1 [MGMT]`) failed with:
...
`//VLAN 1 (Mgmt)/photo   16T  9.4T  6.2T  61% /mnt/truenas-photo`
