...
1. **`MGMT` (Management Zone)**
   * **VLAN ID:** `10`
   * **Gateway IP/Subnet:** `VLAN 10 (MGMT/SecOps)/24`
   * **DHCP Range:** `VLAN 10 (MGMT/SecOps) - VLAN 10 (MGMT/SecOps)`
...
1. Select ports connected to hypervisor nodes:
   * **Bulakan** (Port 7 / Bond)
   * **Cebu** (Port connected to `VLAN 1 [MGMT]`)
   * **Dapitan** (Port connected to `VLAN 1 [MGMT]`)
...
2. **IP Group: `Storage Hosts`**
   * Addresses: `VLAN 1 [MGMT-NAS]`, `VLAN 1 [MGMT-NAS]`, `VLAN 1 (Mgmt)`, `VLAN 10 (MGMT/SecOps)`, `VLAN 10 (MGMT/SecOps)`
3. **IP Group: `DNS Resolvers`**
   * Addresses: `VLAN 1 [DNS-Secondary]`, `VLAN 1 (Mgmt)`, `VLAN 110 (Services)`, `VLAN 110 (Services)`
