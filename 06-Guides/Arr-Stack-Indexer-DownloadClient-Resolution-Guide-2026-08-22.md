...
1. **Legacy IP Mappings in Prowlarr & Sonarr:**
   - When the Arr Stack LXC container (CT 417) was migrated from VLAN 1 (`VLAN 1 (Mgmt)`) to SERVICES VLAN 110 (`VLAN 110 (Services)`), Prowlarr application sync configurations and Sonarr Torznab indexer URLs still retained the old `http://VLAN 1 (Mgmt):9696` and `http://VLAN 1 (Mgmt):8989` endpoints.
   - Sonarr attempts to contact Prowlarr for RSS feeds and search queries failed with `System.Net.Sockets.SocketException (113): Host is unreachable (VLAN 1 (Mgmt):9696)`, placing all indexers in a disabled backoff state.
