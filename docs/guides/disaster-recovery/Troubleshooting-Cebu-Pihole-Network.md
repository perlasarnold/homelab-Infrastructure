# 🛠️ Troubleshooting: Cebu Pi-hole (192.168.1.5) Unreachable

**Date:** 2026-05-21
**Objective:** Resolve issue where `http://192.168.1.5/admin` (Cebu Pi-hole) was not loading and the IP was completely unreachable from the network.

## Problem Statement
The user reported that the secondary Pi-hole web interface on the Cebu node (`192.168.1.5`) was not loading. Pings to the IP address from the user's machine were failing with "Destination Host Unreachable".

## Investigation Steps
1. **Host Verification:** Pinged the Proxmox host `192.168.1.26` (Cebu) which responded successfully, confirming the physical node was online.
...
## Root Cause
The `pihole_cebu` Terraform module deployed the container as an `unmanaged` OS on the `vnet1` bridge. Because it was unmanaged, Proxmox did not auto-generate the `/etc/network/interfaces` file. Without this file, Debian did not bring up the `eth0` interface or assign the static IP (`192.168.1.5`).
...
1. **Bridge Fix:** Changed the container's network interface to use `vmbr0`:
   ```bash
   pct set 401 -net0 name=eth0,bridge=vmbr0,gw=192.168.1.1,ip=192.168.1.5/24
   ```
2. **Interface Configuration:** Manually created the `/etc/network/interfaces` file inside the container to define the static IP:
   ...
   iface eth0 inet static
       ...
       gateway 192.168.1.1
   ```
3. **Service Restart:** Rebooted the container. `eth0` successfully came online with `192.168.1.5`.

## Next Steps & Preventive Measures
- The Pi-hole software has now been successfully installed via the automated setup script. The web interface is accessible at `http://192.168.1.5/admin` with the password `marziel24`.
