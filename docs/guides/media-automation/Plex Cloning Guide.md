...
- **Primary Host**: Bulakan (`VLAN 1 [Management]`) -> Plex Container `104` (`VLAN 1 [Management]`)
- **Secondary Host**: Cebu (`VLAN 1 [Management]`) -> Plex Container `109` (`VLAN 1 [Management]`)
...
```bash
rsync -avzh --progress --exclude='Cache' --exclude='*.db*' /Bulakan-ZFS/subvol-104-disk-1/var/lib/plexmediaserver/Library/ root@VLAN 1 [Management]:/var/lib/plexmediaserver/Library/
```
...
scp -o StrictHostKeyChecking=no /Bulakan-ZFS/subvol-104-disk-1/tmp/plex_backup.db root@VLAN 1 [Management]:'/var/lib/plexmediaserver/Library/Application Support/Plex Media Server/Plug-in Support/Databases/com.plexapp.plugins.library.db'
```
...
Connected the media directories from the Synology NAS (`PNAS` at `VLAN 1 [Management]`) and TrueNAS (`VLAN 1 (Management)`) to the Cebu Host and bind-mounted them to the Cebu LXC to match the exact library paths expected:
...
1. Access the web dashboard at: **[http://VLAN 1 [Management]:32400/web](http://VLAN 1 [Management]:32400/web)**.
